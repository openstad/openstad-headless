...
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| 1   | `logoAltText` → "OpenStad.org, naar de homepage" (Algemene instellingen); dat is meteen de naam van de link                                                            | Sitebreed — header- en footerconfig                                                                                                                                                                                                                                                                                               | 2.4.4                                                                                                      |
| 2   | "Voorbeeld …"-koppen van h3 naar **h2** — `/aftelbalk` en `/reacties` zijn **gedaan 14-08**, de rest nog niet                                                          | Nog te doen: `/agenda`, `/beeldkiezer`, `/begrootmodule`, `/emoji-slider`, `/enquete`, `/interactieve-afbeelding`, `/interactieve-kaart`, `/keuzeswipe`, `/keuzewijzer`, `/likes`, `/stemmodule`, `/teller`, `/verdeelmodule`, `/inzending-formulier`, `/inzending-detailpagina`, `/inzendingen-overzicht`, `/projectenoverzicht` | 1.3.1 — een `h3` onder de `h2`-accordions leest als "Voorbeeld 1 hoort bij Hoe maak je de widget op maat?" |
...
| 11  | **Ná de deploy:** kopniveau per widget zetten — één niveau onder de "Voorbeeld"-kop erboven, dus **h3** zodra actie 2 gedaan is (en **h4** zolang die kop nog `h3` is) | `/interactieve-afbeelding`, `/inzending-detailpagina`, `/begrootmodule`, `/stemmodule` — admin, select "Kopniveau van de titel"                                                                                                                                                                                                   | 1.3.1                                                                                                      |

Al gedaan door de redacteur: logo-`alt` staat inmiddels op "OpenStad.org (logo)", de
`<strong>`-kop is een echte `H3`, de paginatitels zijn beschrijvend, de tijdlijn op
/contentwidgets heeft sinds 14-08 `aria-current="step"` op de tweede fase, en `/sitemap`
staat live met alle 31 links (footer wijst er sinds 14-08 naartoe).
...
| Bevinding                                                            | Status | Toelichting                                                                                                                                                                                                    |
| -------------------------------------------------------------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Logo-link mist een naam                                              | ✍️     | De link heeft wél een naam — die komt uit de `alt` van het logo ("OpenStad.org (logo)"), er is geen `title` nodig of mogelijk. Wat mist is het **doel**: zet `logoAltText` op "OpenStad.org, naar de homepage" |
