...
> e-mailfoutmelding (`ecf9f99f0`), `aria-describedby` (`7593441f4`) en de accountvelden
> (`c1b7f4a66`) — zitten er nog niet in.
...
| 11    | **Nu aan de beurt — de deploy is er.** Kopniveau per widget op **h3** zetten. De titelfix is live, dus widgettitels staan nu op `h2`: precies hetzelfde niveau als de "Voorbeeld"-kop erboven, waardoor ze ernaast lijken te staan in plaats van eronder | Alle 19 widgetpagina's zijn in de DOM nagemeten; zeven raken het: `/begrootmodule`, `/interactieve-afbeelding`, `/interactieve-kaart`, `/inzending-detailpagina`, `/inzendingen-overzicht`, `/projectenoverzicht` en `/stemmodule`. Admin → widget → select "Kopniveau van de titel". `/keuzewijzer` staat wél goed | 1.3.1                                          |
...
| Keuzewijzer: paneel bedekt het scherm, niet te sluiten | ❌      | **Stond hier ten onrechte op ✅. Nog niet opgelost.** De `max-height` + `overflow-y: auto` maakt het paneel intern scrollbaar, maar dat was de helft van de bevinding. De andere helft — het paneel wegkrijgen — werkt niet. Gemeten op 640px: de knop "Details verbergen" wisselt netjes naar "Details bekijken" en `aria-expanded` naar `false`, maar het paneel blijft 604px hoog met alle vier de afbeeldingen zichtbaar. Het paneel is `position: fixed` en beslaat 604×653 in een viewport van 640×700, dus vrijwel het hele scherm. De inklap-CSS (`grid-template-rows: 0fr` op `.expand-container`) is wél gedeployd en de selector matcht, maar de rij krimpt niet: zelfs `grid-template-rows: 0px !important` via inline stijl levert nog steeds 604px op. Zit dus in `packages/choiceguide/src/includes/sidebar.tsx` + `style.css:380-403` en vraagt lokaal uitzoeken |
...
| Keuzewijzer en accountgegevens                               | ❌ / ✅ | Volgen 1.4.4. Accountgegevens is op 320px en 640px nagemeten en opgelost. **Keuzewijzer niet**: het paneel is niet weg te klikken, zie 1.4.4                                                       |
