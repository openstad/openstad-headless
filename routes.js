...
//	app.get('/register/info')
  ...
  app.get('/dialog/authorize',            clientMw.withOne, authMw.check, userMw.withRoleForClient, clientMw.checkIfEmailRequired(), clientMw.checkRequiredUserFields,  clientMw.checkUniqueCodeAuth((req, res) => { return res.redirect('/login?clientId=' + req.query.client_id);}),   oauth2Controller.authorization);
  app.post('/dialog/authorize/decision',  clientMw.withOne, clientMw.checkUniqueCodeAuth(), clientMw.checkIfEmailRequired(), bruteForce.global.prevent, oauth2Controller.decision);
...
//
  app.get('/api/userinfo', passport.authenticate('bearer', { session: false }), clientMw.withOne, clientMw.checkUniqueCodeAuth(),  clientMw.checkIfEmailRequired(), userMw.withRoleForClient, userController.info);
