...
export const calculateColor = (
  ...
  if (maxColorMatch && minColorMatch) {
    const r =
      parseInt(minColorMatch[1], 16) +
      (parseInt(maxColorMatch[1], 16) - parseInt(minColorMatch[1], 16)) *
        (score / 100);
    const g =
      parseInt(minColorMatch[2], 16) +
      (parseInt(maxColorMatch[2], 16) - parseInt(minColorMatch[2], 16)) *
        (score / 100);
    const b =
      parseInt(minColorMatch[3], 16) +
      (parseInt(maxColorMatch[3], 16) - parseInt(minColorMatch[3], 16)) *
        (score / 100);
    return `rgb(${r},${g},${b})`;
  }
  return '#bed200';
  ...
  choiceOptionsArray.forEach((option) => {
    Object.keys(weights[option.id] || {}).forEach((answerKey) => {
      ...
      ['x', 'y', 'z'].forEach((dimension) => {
        if (
          typeof optionWeights[dimension] === 'number' &&
          optionWeights[dimension] !== 0
        ) {
      ...
      Object.keys(optionWeights).forEach((optionId) => {
        ...
        const isImageChoiceMultiple =
          isImageChoice &&
          items?.find(
            (item) =>
              item?.trigger === imageChoiceTrigger && item?.multiple === true
          );
        ...
        ['x', 'y', 'z'].forEach((secondDimension) => {
          ...
          if (typeof optionValue === 'number' && optionValue !== 0) {
            if (isRadioBox || (isImageChoice && !isImageChoiceMultiple)) {
              tempScores[secondDimension] = Math.max(
                tempScores[secondDimension],
                optionValue
              );
            } else {
      ...
      answerArray.forEach((userAnswer) => {
        const safeUserAnswer =
          typeof userAnswer === 'string'
            ? userAnswer?.replace(/\./g, '_DOT_')
            : userAnswer;
        const optionWeights = weights[option.id]?.[answerKey];

        if (optionWeights) {
          if (
            typeof safeUserAnswer === 'string' &&
            optionWeights[safeUserAnswer]
          ) {
            ...
            Object.keys(optionWeights).forEach((fifthDimension) => {
              ...
              const optionsHasAB =
                fifthDimension === 'x' &&
                optionWeights.hasOwnProperty('ab') &&
                optionWeights.ab === 'A';
              const optionsHasABY =
                fifthDimension === 'y' &&
                optionWeights.hasOwnProperty('aby') &&
                optionWeights.aby === 'A';

              const reverseValue = optionsHasAB || optionsHasABY;
              const valueBasedOnAB = reverseValue
                ? 100 - fieldValue
                : fieldValue;
