...
const getExistingValue = (fieldKey, resource, multiple) => {
  if (!!resource) {
    ...
    const returnField =
      !field && resource.extraData
        ? resource.extraData[fieldKey] || null
        : field;
...
function ResourceFormWidget(props: ResourceFormWidgetProps) {
  ...
  const addTagsToFormData = (formData) => {
        ...
        if (key.startsWith('tags[')) {
          try {
            ...
            if (typeof tagsArray === 'object') {
              ...
              typeof tagsArray === 'number'
            ) {
              const pushValue =
                typeof tagsArray === 'string' ? Number(tagsArray) : tagsArray;
              tags.push(pushValue);
            }
          } catch (error) {
    ...
    if (defaultAddedTags) {
      const defaultTagsArray = defaultAddedTags
        .split(',')
        .map((tag) => tag.trim())
  ...
  const redirectAfterSaveOrCreate = (
    resource: { id?: string },
    reloadPageAsFallback = false
  ) => {
  ...
  const paginationFields = formFields.filter(
