...
const SwipeField: FC<SwipeWidgetProps> = ({
  ...
  const validateOverrideData = useCallback(
    (override: FormValue | undefined): boolean => {
      if (
        !override ||
  ...
  const [swipeAnswers, setSwipeAnswers] = useState<Record<string, string>>(
    () => getInitialAnswers().initialAnswers
  );
  const [explanations, setExplanations] = useState<Record<string, string>>(
    () => getInitialAnswers().initialAnswersExplanation
  );
  ...
  const [animationType, setAnimationType] = useState<'drag' | 'button' | null>(
    null
  );
  ...
  const [infoVisibleCardId, setInfoVisibleCardId] = useState<string | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const initialCardIdsString = Array.from(initializedCardIds.current)
      .sort()
      .join(',');
  ...
  useEffect(() => {
    ...
    if (
      overrideDefaultValue &&
      Array.isArray(overrideDefaultValue) &&
      overrideDefaultValue.length > 0
    ) {
      const overrideArray = overrideDefaultValue as valueObject;

      // Check if ANY of the cardIds in override belong to another component
      const hasOtherComponentData = overrideArray.some(
        (item) => !componentCardIds.has(item.cardId)
      );
      const hasOwnComponentData = overrideArray.some((item) =>
        componentCardIds.has(item.cardId)
      );
  ...
  useEffect(() => {
    if (onChange) {
      const combinedAnswers: valueObject = swipeCards
        .map((card) => ({
          cardId: card.id,
  ...
  if (isFinished || unansweredCards.length === 0) {
    return (
      <>
        ...
        <div
          className="swipe-widget swipe-finished"
          role="region"
          aria-live="polite"
          tabIndex={0}>
                  ...
                  <div key={card.id} className="swipe-summary-item">
                    ...
                    <div className="swipe-summary-explanation">
                      <textarea
                        ...
                        onChange={(e) =>
                          handleExplanationChange(card.id, e.target.value)
                        }
                        rows={3}
                      />
                    </div>
  ...
  return (
    <>
      ...
      <div
        className="swipe-widget"
        role="region"
        aria-label="Swipe widget"
        tabIndex={0}
        aria-invalid={
          !required && Object.keys(swipeAnswers).length === 0 ? 'false' : 'true'
        }
                ...
                <>
                  <div
                    ...
                    <div className="swipe-card-content">
                      <Paragraph
                        className="swipe-card-description"
                        id={`swipe-card-desc-${card.id}`}>
                        {card.title}
                      </Paragraph>
                    </div>
          ...
          <div className="swipe-actions" role="group" aria-label="Acties">
            ...
            <div className="swipe-middle-actions">
              <button
                ...
                onClick={(e) => {
                  e.preventDefault();
                  setInfoVisibleCardId(
                    infoVisibleCardId === currentCardId ? null : currentCardId
                  );
                }}
              ...
              <button
                className="swipe-skip-btn"
                onClick={(e) => (e.preventDefault(), moveToNext())}
                aria-label="Kaart overslaan"
                disabled={required}>
                <span>Overslaan</span>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
