...
function DocumentMap({
  ...
  const { tagsString: filteredTagsIdsString, tags: filteredTagIdsArray } =
  ...
  const [selectedTags, setSelectedTags] =
    useState<Array<number>>(urlTagIdsArray);
  ...
  useEffect(() => {
    const selectedTagsForFiltering = Array.isArray(selectedTags)
      ? selectedTags
      : [];
    const originalTagsForFiltering = Array.isArray(filteredTagIdsArray)
      ? filteredTagIdsArray
      : [];
    const allTagsToFilter =
      selectedTagsForFiltering.length > 0
        ? selectedTagsForFiltering
        : originalTagsForFiltering;

    const finalAllTagsToFilter = allTagsToFilter.map((tag: string | number) =>
  ...
  useEffect(() => {
    ...
    const extendedBounds: LatLngBoundsLiteral = [
      [basicBounds[0][0] + docHeight * 0.2, basicBounds[0][1] + docWidth * 0.2],
      [basicBounds[1][0] - docHeight * 0.2, basicBounds[1][1] - docWidth * 0.2],
    ];
  ...
  const characterHelpText = (count: number) => {
    ...
    const minWarningText =
      props?.minCharactersWarning || 'Nog minimaal {minCharacters} karakters';
    let minWarning = minWarningText.replace(
      '{minCharacters}',
      (min - count).toString()
    );
  ...
  const notifySuccess = () =>
    NotificationService.addNotification(
      'Uw reactie is succesvol geplaatst!',
      'success'
    );
  const notifyFailed = () =>
    NotificationService.addNotification(
      'Uw reactie kon niet geplaatst worden',
      'error'
    );
    ...
    if (
      ...
      try {
        ...
        const allTags = Array.from(
          new Set([...defaultTagsArray, ...selectedOptions])
        );
              ...
              <div className="url-container">
                <div className="url-list">
                  {accessibilityUrlVisible ? (
                    <Link
                      href={getUrl()}
                      title="Bekijk tekstuele versie"
                      id={randomId}>
                      {props.accessibilityUrlText}
                    </Link>
                  ) : null}
                  {definitiveUrlVisible &&
                  originalID !== undefined &&
                  isDefinitive ? (
                    <Link
                      href={getDefinitiveUrl(originalID)}
                      title="Bekijk originele versie"
                      id={randomId}>
                      {props.definitiveUrlText}
                    </Link>
                  ) : null}
                </div>
              </div>
                ...
                {!hasRole(currentUser, args.requiredUserRole) ? (
                  ...
                  <form>
                    <div>
                      ...
                      {helpText && (
                        <Paragraph className="--error">{helpText}</Paragraph>
                      )}
            ...
            <>
              <Button
                ...
                <span className="sr-only">
                  {infoPopupButtonText || 'Hoe werkt het?'}
                </span>
              </Button>
