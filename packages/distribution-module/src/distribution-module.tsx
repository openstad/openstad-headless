...
export type DistributionModuleProps = BaseProps &
  ProjectSettingProps & {
    widgetId?: number;
...
function DistributionModule(props: DistributionModuleProps) {
  const notifyCreate = () =>
    NotificationService.addNotification('Verdeling ingediend', 'success');
  ...
  const valuesChanged = (values: { [p: string]: FormValue }) => {
    const checkIfValuesAreNotEmpty = Object.values(values).filter(
    ...
    const valuesTotal = checkIfValuesAreNotEmpty.reduce(
      (acc: number, value) => acc + parseInt(value || '0'),
      0
    );
