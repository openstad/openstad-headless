...
export default function useIdeas(props) {
  ...
  ideas.create = function(newData) {
    return self.mutate({ projectId }, 'ideas.create', newData, { action: 'create' });
  }
  ideas.filter = function(filter) {
    return self.mutate({ projectId }, 'ideas.fetch', null, { action: 'fetch',  filter });
  }
  ideas.map( async idea => {
    idea.update = function(newData) {
      return self.mutate({ projectId }, 'ideas.update', newData, { action: 'update' });
    }
    idea.delete = function(newData) {
      return self.mutate({ projectId }, 'ideas.delete', idea, { action: 'delete' });
    }
    idea.submitLike = function() {
      return self.mutate({ projectId }, 'ideas.submitLike', idea, { action: 'update' });
    }
  })
