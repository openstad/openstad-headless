...
function Activity({
  ...
  const userId = sessionData
    ? JSON.parse(sessionData)[props.projectId]?.openStadUser?.id
    : null;
  ...
  const listItem = (data: ActivityData, key: number) => {
    ...
    const truncatedDescription =
      data.description.length > truncate
        ? `${data.description.substring(0, truncate)}...`
        : data.description;
    return (
      <li key={key} className="activity__item">
        <Heading level={3} className="activity__header">
          {data.title}{' '}
          <span className="activity__header--label">{data.label}</span>
        </Heading>
        <Paragraph className="activity__description">
          {truncatedDescription}
        </Paragraph>
        <div>
          <Paragraph className="activity__siteTitle">{data.site}</Paragraph>
          <Paragraph className="activity__date">
  ...
  return (
    <section className="user-activity">
      <div>
        <Heading level={2}>
          {currentTitle ? currentTitle : 'Activiteit op deze website'}
        </Heading>

        <ul className="user-acivity__list">
          {activityData === undefined ||
            activityData.currentSite === undefined ||
            (activityData.currentSite.length === 0 &&
              noActivity(
                noActivityTextCurrent
                  ? noActivityTextCurrent
      ...
      <div>
        <Heading level={2}>
          {otherTitle ? otherTitle : 'Activiteit op andere websites'}
        </Heading>
        <ul className="user-acivity__list">
          {activityData === undefined ||
            activityData.otherSites === undefined ||
            (activityData.otherSites.length === 0 &&
              noActivity(
                noActivityTextOther
                  ? noActivityTextOther
