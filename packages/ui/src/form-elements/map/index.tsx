...
const MapField: FC<MapProps> = ({
  ...
  const resolvePolygonTagsForLocation = (location?: {
    lat?: number;
    lng?: number;
  }) =>
    resolvePolygonTags({
    ...
    <FormField type="text">
      ...
      {!!props.showHiddenPolygonsForAdmin && hasHiddenAdminPolygons && (
        <div className="map-hidden-polygons-message">
          <Paragraph>
            Let op: polygonen met rode vulling en een gestippelde rand zijn
            verborgen voor gebruikers aan de voorkant. Deze zijn alleen
            zichtbaar voor beheerders in de admin omgeving.
          </Paragraph>
        </div>
      )}
      ...
      <div className="form-field-map-container" id={`map`}>
        {((allowedPolygonIds.length > 0 && polygon.length) ||
          ...
          <EditorMap
            ...
            areaRenderMode={
              props.areaRenderMode ||
              (allowedPolygonIds.length && allowedPolygonIds.length > 1)
                ? 'polygons'
                : 'cutout'
            }
            adminOnlyPolygons={
              !!props.showHiddenPolygonsForAdmin ? adminOnlyPolygons : undefined
            }
            {...zoom}
