...
const MatrixField: FC<MatrixFieldProps> = ({
  ...
  const handleChoiceChange = (value: string, rowId: string): void => {
    const removeSelectedRadio =
      !matrixMultiple &&
      selectedChoices.find((choice) => choice.startsWith(rowId));
    const removeSelectedCheckbox =
      matrixMultiple && selectedChoices.includes(value);

    let newSelectedChoices = [...selectedChoices];
    if (removeSelectedRadio) {
      newSelectedChoices = newSelectedChoices.filter(
        (choice) => !choice.startsWith(rowId)
      );
                            ...
                            {matrixMultiple ? (
                              <>
                                <Checkbox
                                  ...
                                  onChange={() =>
                                    handleChoiceChange(cellIndex, rowIndex)
                                  }
                                  disabled={
                                    disabled ||
                                    (maxReached &&
