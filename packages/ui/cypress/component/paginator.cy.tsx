...
describe('<Paginator />', () => {
  ...
  it('renders and call onPageChange', () => {
    ...
    cy.mount(<Paginator onPageChange={onPageChange} page={0} totalPages={2} />);

    cy.get('.osc-paginator').should('exist');

    cy.get('[test-id="previous-page-button"]')
      .should('exist')
      .should('be.disabled');
    cy.get('[test-id="next-page-button"]')
      .should('exist')
      .should('not.be.disabled')
      .click();

    cy.get('[test-id="page-button-0"]').should('exist').should('be.disabled');
    cy.get('[test-id="page-button-1"]')
      .should('exist')
      .should('not.be.disabled');
