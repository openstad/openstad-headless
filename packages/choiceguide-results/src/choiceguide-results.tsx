...
function ChoiceGuideResults(props: ChoiceGuideResultsProps) {
  ...
  useEffect(() => {
    ...
    const initialWeights = InitializeWeights(
      ...
      choiceGuide?.choicesType || 'default',
      hiddenFields
    );
  ...
  const hideScores =
    props?.hideScores === true && props?.displayAsFeaturedOnly === true;
  const [bestScoredChoiceOptions, setBestScoredChoiceOptions] = useState<
    ChoiceOptions[]
  >([]);

  useEffect(() => {
    if (
      !props?.displayAsFeaturedOnly ||
      !items?.length ||
      !choiceOption?.choiceOptions ||
      !weights
    )
    ...
    if (choiceType === 'plane') {
      ...
      const planeConditions = [
        {
          condition: planeScore.x <= 50 && planeScore.y > 50,
          option: options[0],
        },
        {
          condition: planeScore.x > 50 && planeScore.y > 50,
