...
class AzureTransport {
  ...
  async send(mail, callback) {
    try {
      const { subject, from, to, text, html, attachments = [] } = (mail && mail.data) || {};
      ...
      const toRecipients = (Array.isArray(to) ? to : [to]).map((recipient) => ({ emailAddress: { address: this.returnOnlyEmail(recipient) } }));
      ...
      const response = await fetch(`${this.graphEndpoint}/v1.0/users/${this.returnOnlyEmail(from)}/sendMail`, {
        method: "POST",
        headers: {
