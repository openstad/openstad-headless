const memberRoleId = process.env.MEMBER_ROLE_ID
  ...
  : 2;
const anonymousRoleId = process.env.ANONYMOUS_ROLE_ID
  ? process.env.ANONYMOUS_ROLE_ID
  : 3;
...
const types = [
  {
    ...
    label: 'Mijn stemcode',
    helpText:
      'Let op, de unieke code is hoofdlettergevoelig! Werkt deze nog steeds niet? <a href="mailto:info@openstad.nl">Neem contact met ons op.</a>',
    errorMessage:
  ...
  },
  {
    ...
    loginDescription:
      'Vul uw telefoonnummer in om een sms-stemcode te ontvangen. In stap 2 voert u de sms-code in. Als dat gelukt is komt u terug op de stemsite waar u uw stem kunt insturen. Een sms-code is 60 minuten geldig.',
    ...
    loginErrorMessage:
      'Het is niet gelukt een sms te versturen. Controleer of u een geldig telefoonnummer heeft ingevoerd.',
...
exports.adminTypes = types
  .filter((type) => ['Local', 'Url'].includes(type.key))
  .map((type) => {
