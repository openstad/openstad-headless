...
const privateKey = process.env.PRIVATE_KEY_BASE64
  ...
  : fs.readFileSync(path.join(__dirname, certPath + '/privatekey.pem'));
...
const publicKey = process.env.PUBLIC_KEY_BASE64
  ? Buffer.from(process.env.PUBLIC_KEY_BASE64, 'base64').toString('utf8')
  : fs.readFileSync(path.join(__dirname, certPath + '/certificate.pem'));
