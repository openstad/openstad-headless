...
module.exports = function (app) {
  ...
  //shared middleware
  const localSharedMW = [
    clientMw.setAuthType('Local'),
    csrfProtection,
    addCsrfGlobal,
  ];
  //routes
  app.get(
    '/auth/local/login/:priviligedRoute?',
    [...localSharedMW],
    clientMw.validate,
    authLocal.login
  );
  ...
  app.post(
    ...
    [...localSharedMW],
    passwordResetMw.validate,
    userMw.validatePassword,
    authForgot.postReset
  );
  ...
  // routes
  app.get(
    '/auth/url/login/:priviligedRoute?',
    clientMw.setAuthType('Url'),
    clientMw.validate,
    csrfProtection,
    addCsrfGlobal,
    authUrl.login
  ...
  app.get(
    '/auth/url/confirmation',
    clientMw.setAuthType('Url'),
    csrfProtection,
    addCsrfGlobal,
    authUrl.confirmation
  );
  app.post(
    '/auth/url/login/:priviligedRoute?',
    clientMw.setAuthType('Url'),
   ...
   */
  // shared middleware
  app.use('/auth/anonymous', [
    clientMw.withOne,
    clientMw.setAuthType('Anonymous'),
    clientMw.validate,
    csrfProtection,
    addCsrfGlobal,
  ]);
   ...
   */
  // shared middleware
  app.use('/auth/phonenumber', [
    clientMw.withOne,
    clientMw.setAuthType('Phonenumber'),
    clientMw.validate,
  ]);

  // routes
  app.get(
    '/auth/phonenumber/login',
    csrfProtection,
    addCsrfGlobal,
    authPhonenumber.login
  );
  app.post(
    '/auth/phonenumber/login',
    csrfProtection,
    phonenumberBruteForce,
    authPhonenumber.postLogin
  );
  app.get(
    '/auth/phonenumber/sms-code',
    csrfProtection,
    addCsrfGlobal,
  ...
  app.use('/auth/code', [
    clientMw.withOne,
    clientMw.setAuthType('UniqueCode'),
    clientMw.validate,
  ]);
  ...
  app.post(
    '/auth/code/login',
    csrfProtection,
    uniqueCodeBruteForce,
    logMw.logPostUniqueCode,
    authCode.postLogin
  );
  ...
  app.get(
    '/auth/two-factor/configure',
    clientMw.withOne,
    csrfProtection,
    addCsrfGlobal,
    authTwoFactor.configure
  );
  app.post(
    '/auth/two-factor/configure',
    clientMw.withOne,
  ...
  app.use(async function (err, req, res, next) {
    ...
    if (
      err &&
      err.message &&
      err.message.match(/^No Client ID is set for login/)
    ) {
    ...
    if (err && err.message && err.message.match(/^Error in deserializeUser/)) {
      console.log(
        `[auth-error][${new Date().toISOString()}] deserializeUser error, destroying session and redirecting to logout: ${err?.message}`
      );
