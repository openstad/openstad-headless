      ...
      async load(req, widgets) {
        ...
        for (const widget of widgets) {
          try {
            ...
            let additionalPosts = [];
            if (
              widget.selectionType === 'manual' &&
              widget.selectedPosts &&
              widget.selectedPosts.length > 0
            ) {
              additionalPosts = widget.selectedPosts.filter((post) => {
                const highlightedDoc =
                  widget.highlightedPost && widget.highlightedPost.length > 0
                    ? widget.highlightedPost[0]
                    : null;
                const hasTag =
                  !widget.tag || (post.tags && post.tags.includes(widget.tag));
                return (
                  !highlightedDoc ||
                  (post.slug !== highlightedDoc.slug && hasTag)
                );
              });
            } else {
              const criteria = {};
            ...
            posts = posts.concat(
              showAll
                ? additionalPosts
                : additionalPosts.slice(
                    0,
                    maxItems ? maxItems - posts.length : undefined
                  )
            );
            posts = posts.filter(
              (post, index, arr) =>
            ...
            widget.relatedPosts = posts.map((post) => {
              if (post.createdAt) {
                ...
                post.createdAt = isNaN(dateObj.getTime())
                  ? null
                  : dateObj.toISOString();
              }
              return post;
