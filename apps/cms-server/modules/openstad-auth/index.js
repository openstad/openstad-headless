    ...
    return {
      async enrich(req, res, next) {
        ...
        const encodedUrl = encodeURIComponent(
          `${req.protocol}://${req.hostname}${req.url}`.replace(/'/g, '%27')
        );
      ...
      async authenticate(req, res, next) {
        ...
        if (req.query.openstadlogintoken) {
          ...
          if (!jwt) {
            ...
            const dateToCheck = req.session.openStadlastJWTCheck
              ? new Date(req.session.openStadlastJWTCheck)
              : new Date().getTime() - THIRTY_SECONDS - 1;
      ...
      async aposAuthenticate(req, res, next) {
        ...
        if (self.apos && self.apos.user) {
          ...
          const insertOrUpdate = aposUser
            ? self.apos.user.update
            : self.apos.user.insert;
