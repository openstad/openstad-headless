exports.default = {
  ...
  format: (id, cssValues) => {
    ...
    if (cssValues) {
       ...
       */
      const mainStyles = formatCSSLines(
        cssValues.filter((cssValue) => {
          return !cssValue.mediaQuery;
        })
      );
       ...
       */
      const mediaQueries = cssValues
        .filter((cssValue) => !!cssValue.mediaQuery)
        .map((cssValue) => cssValue.mediaQuery);
      mediaQueries.forEach((mediaQuery) => {
        let mediaQueryStyle = formatCSSLines(
          cssValues.filter(
            (cssValue) =>
              cssValue.mediaQuery && cssValue.mediaQuery === mediaQuery
          )
        );
