...
export default async function middleware(req: NextRequest) {
  ...
  if (req.nextUrl.pathname.match(/^\/signin$/i)) {
    return NextResponse.redirect(
      `${process.env.API_URL}/auth/project/1/login?useAuth=default&redirectUri=${process.env.URL}/projects`
    );
  }
  ...
  if (req.nextUrl.pathname.match(/^\/signout$/i)) {
    ...
    session.destroy();
    return NextResponse.redirect(
      `${process.env.API_URL}/auth/project/1/logout?useAuth=default&redirectUri=${process.env.URL}/`,
      { headers: res.headers }
    );
  }
  ...
  if (match) {
    ...
    if (!HasAccess(session?.user) && restrictedPaths.includes(subPath)) {
      return NextResponse.redirect(
        `${process.env.URL}/projects/${projectId}/widgets`
      );
    } else if (!HasAccess(session?.user) && subPath === '/settings') {
      return NextResponse.redirect(
        `${process.env.URL}/projects/${projectId}/settings/voting`
      );
    }
  }
