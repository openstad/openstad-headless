        ...
        <form
          ...
          {datalayers?.map((item) => (
            <div key={item.id}>
              ...
              {form.getValues('enableOnOffSwitching') &&
                ...
                form
                  .getValues('datalayer')
                  ?.some((obj) => obj.id === Number(item.id)) && (
                  <FormField
                    control={form.control}
                    ...
                    render={({ field }) => {
                      const activeOnInit =
                        form
                          .getValues('datalayer')
                          ?.find((obj) => obj.id === Number(item.id))
                          ?.activeOnInit ?? true;
                      ...
                      return (
                        <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                          ...
                          <FormLabel className="font-normal">
                            Deze laag standaard tonen bij het openen van de
                            kaart
                          </FormLabel>
                          <FormMessage />
                        </FormItem>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>
                  Wil je dat het mogelijk is om de kaartlagen aan en uit te
                  zetten?
                </FormLabel>
                <FormDescription>
                  Als je dit aanvinkt kunnen gebruikers de kaartlagen aan en uit
                  zetten. De kaartlagen worden getoond in een legenda.
                </FormDescription>
