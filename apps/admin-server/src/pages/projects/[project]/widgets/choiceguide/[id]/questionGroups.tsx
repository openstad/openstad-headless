...
export default function WidgetChoiceGuideQuestionGroups(props) {
  const category = 'questionGroup';
  ...
  const defaults = useCallback(
    () => {
      const questionGroups = widget?.config?.[category]?.questionGroups || [];
      ...
      if (questionGroups.length > 0) {
        nextIdRef.current = Math.max(...questionGroups.map(group => group.id)) + 1;
      } else {
        nextIdRef.current = 1;
      }
      return { questionGroups, weights };
    },
    [widget?.config]
  );
  ...
  async function onSubmit(values: FormData) {
    ...
    const updatedConfig = {
      ...widget.config,
      [category]: { questionGroups: values.questionGroups },
      items: updatedItems
    };
                ...
                items={[
                  {
                    ...
                    content: (
                      <div className="flex flex-col w-full col-span-full mt-4 mb-4 bg-gray-100 rounded-md p-6">
                        ...
                        <FormField
                          control={form.control}
                          name={`questionGroups.${index}.title`}
                          render={({field}) => (
                            <FormItem>
                        ...
                        <FormField
                          control={form.control}
                          name={`questionGroups.${index}.description`}
                          render={({field}) => (
                            <FormItem>
                              ...
                              <div className="p-0 flex flex-col justify-between col-span-2">
                                ...
                                <div className="w-full mt-4 flex flex-col gap-y-4">
                                  {widget?.config?.items
                                    ...
                                    if (['radiobox', 'checkbox'].includes(item.type)) {
