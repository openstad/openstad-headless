...
export default function ProjectSettingsWidgets() {
  ...
  const {
    ...
    defaultValues: {
      urls:
        (data?.config?.allowedDomains ?? []).map((url: any) => ({ url })) || [],
    },
  });
  ...
  useEffect(() => {
    if (data?.config?.allowedDomains) {
      ...
      reset({
        urls: (data?.config?.allowedDomains ?? []).map((url: any) => ({ url })),
      });
    }
  }, [data, reset]);
        ...
        <div className="container py-6">
          <Form className="p-6 bg-white rounded-md">
            ...
            <p className="text-gray-500">
              Hier kun je de URL’s instellen waar de widgets van dit project op
              mogen draaien.
            </p>
            <p className="text-gray-500">
              Let op: Widgets werken alleen op externe websites als de URL van
              de betreffende website hier is toegevoegd.
            </p>
            <br />
            <form
