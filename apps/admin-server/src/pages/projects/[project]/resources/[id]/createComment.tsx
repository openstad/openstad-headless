...
export default function ProjectResourceCreateArgument() {
  ...
  const notifyFailed = (err?: string) => toast.error(err ||"Reactie plaatsen mislukt");
  const notifyError = () => toast.error("Er is een fout opgetreden");
  const notify = (message: string, icon: any) => toast(message, { icon, duration: 8000 });
  ...
  async function onSubmit(values: Formdata) {
    ...
    try {
      ...
      if ( !newComment?.error ) {
        ...
        if (values.confirmation) {
          let icon = '✅', text = 'Notificatie naar gebruiker is verzonden';
          if (newComment.confirmationSent === false) {
            icon = '⚠️', text = 'Notificatie naar gebruiker is niet verzonden, omdat de gebruiker geen toestemming heeft gegeven voor e-mailnotificaties.';
          } else if (newComment.confirmationSent === undefined) {
            icon = '❌', text = 'Notificatie naar gebruiker kon niet worden verzonden';
          }
        ...
        <form
          ...
          <FormField
            ...
            render={({field}) => (
              <FormItem>
                <FormLabel>
                  Bevestingingsmail verzenden?
                </FormLabel>
                <FormDescription>
                  Wil je een bevestigingsmail sturen naar de gebruiker die de inzending heeft ingediend? De gebruiker ontvangt dit alleen als hij/zij toestemming heeft gegeven voor e-mailnotificaties.
                </FormDescription>
                <Select
