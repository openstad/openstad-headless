...
export default async function getBrokerConfigurationFromUrl(
  ...
  if (
    ...
    data?.id_token_signing_alg_values_supported.length > 0
  ) {
  ...
  if (!issuer) {
    return res
      .status(400)
      .json({ error: 'Invalid issuer in broker configuration' });
  }
  ...
  for (const [key, value] of Object.entries(mapping)) {
    if (data[key]) {
      ...
      if (url.hostname === issuerUrl.hostname) {
        ...
        return res
          .status(400)
          .json({ error: `${key} is invalid and does not contain issuer URL` });
      }
    }
  }
  ...
  sanitizedData.serverLoginPath =
    sanitizedData.serverLoginPath +
    `?client_id=[[clientId]]&redirect_uri=[[redirectUri]]&response_type=code&scope=openid%20irma-demo.sidn-pbdf.uniqueid.uniqueid&code_challenge=[[codeChallenge]]&code_challenge_method=S256&response_mode=query`;
  sanitizedData.serverExchangeContentType =
    data?.token_endpoint_auth_methods_supported?.includes('client_secret_post')
