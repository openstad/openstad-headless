...
export default async function getBrokerConfigurationFromUrl(req: NextRequest | NextApiRequest, res: NextApiResponse) {
  ...
  sanitizedData.serverLoginPath = sanitizedData.serverLoginPath + `?client_id=[[clientId]]&redirect_uri=[[redirectUri]]&response_type=code&scope=openid%%20irma-demo.sidn-pbdf.uniqueid.uniqueid&code_challenge=[[codeChallenge]]&code_challenge_method=S256&response_mode=query`;
