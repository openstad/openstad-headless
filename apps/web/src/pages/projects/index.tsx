          ...
          >
            <div className="container mx-auto py-10">
              <div className="mt-4 grid grid-cols-2 md:grid-cols-12 items-center py-2 px-2 border-b border-border overflow-x-scroll">
                {headers.map(header =>   <ListHeading className="hidden md:flex" key={header}>
                  {header}
                </ListHeading>)}
              </div>
              <ul>
                {data.map((project: any) => {
                  return (
                    <li className='grid grid-cols-2 md:grid-cols-12 items-center py-3 px-2 h-16 hover:bg-secondary-background hover:cursor-pointer border-b border-border gap-2' key={project.id} onClick={(d) =>{
