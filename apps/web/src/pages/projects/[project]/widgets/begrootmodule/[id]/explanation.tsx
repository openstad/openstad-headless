...
export default function BegrootmoduleExplanation() {
  ...
  const defaults = () => ({
    step1:
      widget?.config?.explanations?.step1 ||
    ...
    step3success:
      widget?.config?.explanations?.step3success ||
      "Het controleren van je stemcode is gelukt! Je bent bijna klaar. Klik op onderstaande knop om je stem te versturen.",
    voteMessage:
      widget?.config?.explanations?.voteMessage || "Gelukt, je hebt gestemd!",
    thankMessage:
      widget?.config?.explanations?.thankMessage ||
    ...
    showNewsletterButton:
      widget?.config?.explanations?.showNewsletterButton || false,
  });
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    updateConfig({ explanations: values });
  }
