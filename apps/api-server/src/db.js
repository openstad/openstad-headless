...
if (dbConfig.mysqlCaCert?.trim?.()) {
  ...
  const caCert = dbConfig.mysqlCaCert.trim().indexOf('-----BEGIN CERTIFICATE-----') === 0 ? dbConfig.mysqlCaCert.trim() : Buffer.from(dbConfig.mysqlCaCert.trim(), 'base64').toString('utf8');
