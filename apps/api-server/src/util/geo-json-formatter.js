...
exports.formatPolygonToGeoJson = (polygons) => {
  ...
  if (isMultiPolygon) {
    ...
    const features = polygons.map(polygon => ({
      ...
      "geometry": {
        "type": "Polygon",
        "coordinates": polygon.map(ring => ring.map(point => [point.lng, point.lat]))
      }
    }));
    ...
    const feature = {
      ...
      "geometry": {
        "type": "Polygon",
        "coordinates": polygons.map(ring => ring.map(point => [point.lng, point.lat]))
      }
    };
...
exports.formatGeoJsonToPolygon = (geoJSON) => {
  ...
  const result = geoJSON.features.flatMap(feature => {
    ...
    if (feature.geometry.type === 'MultiPolygon') {
      ...
      const totalPoints = polygons.reduce((sum, p) => sum + p.reduce((s, r) => s + r.length, 0), 0);
