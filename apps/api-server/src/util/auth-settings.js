...
let getConfig = async function ({ project, useAuth = 'default', req }) {
  ...
  if (
    ...
    project.config.authProviders.includes(parseInt(useAuthProvider, 10))
  ) {
    // log trace
    let authProvider = await db.AuthProvider.findOne({
      where: { id: useAuthProvider },
    });
    if (authProvider) {
      let providerConfig = authProvider.config || {};
      let adapterConfig = projectConfig.adapter[authProvider.type] || {};
      authConfig = merge.recursive(
        authConfig,
        providerConfig,
        adapterConfig,
        { adapter: authProvider.type },
        { authProviderId: useAuthProvider }
      );
    }
  } else {
