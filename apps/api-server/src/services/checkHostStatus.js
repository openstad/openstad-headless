          ...
          paths: [
            {
              backend: {
                service: {
                  name:
                    process.env.KUBERNETES_FRONTEND_SERVICE_NAME ||
                    'openstad-frontend',
                  port: {
                    number: process.env.KUBERNETES_FRONTEND_SERVICE_PORT
                      ? parseInt(process.env.KUBERNETES_FRONTEND_SERVICE_PORT)
                      : 4444,
                  },
                },
              },
...
const createIngress = async (
  ...
  const prodIssuerName = useClusterIssuer
    ? process.env.KUBERNETES_INGRESS_ISSUER_NAME || 'openstad-letsencrypt-prod'
    : null;

  const defaultAnnotations = process.env.KUBERNETES_INGRESS_DEFAULT_ANNOTATIONS
  ...
  if (
    useClusterIssuer &&
    !!prodIssuerName &&
    !defaultAnnotations['cert-manager.io/cluster-issuer']
  ) {
