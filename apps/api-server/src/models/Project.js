...
module.exports = function (db, sequelize, DataTypes) {
      ...
      installationUrls: {
        ...
        get() {
          return {
            ...
            img: process.env.IMAGE_APP_URL
              ? new URL(process.env.IMAGE_APP_URL).host
              : '',
          };
        },
        set(val) {
          throw new Error(
            'installationUrls is a virtual field and cannot be set'
          );
        },
        auth: {
          viewableBy: 'editor',
      ...
      hooks: {
        beforeValidate: async function (instance, options) {
          try {
            ...
            let current = await db.Project.findOne({
  ...
  async function beforeUpdateOrCreate(instance, options) {
    ...
    if (
      ...
      instance.set(
        'config',
        merge.recursive(true, instance.config, { uniqueId })
      );
    }
  }
  ...
  Project.associate = function (models) {
    ...
    this.hasMany(models.NotificationTemplate, {
      onDelete: 'CASCADE',
      hooks: true,
    });
  ...
  Project.prototype.willAnonymizeAllUsers = async function () {
    ...
    try {
      ...
      let users = await db.User.findAll({
        where: {
          projectId: self.id,
          idpUser: { identifier: { [Sequelize.Op.ne]: null } },
        },
      });
