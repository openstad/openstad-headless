...
module.exports = function (db, sequelize, DataTypes) {
  ...
  User.prototype.hasConfirmed = function () {
    ...
    return db.Vote.findOne({
      where: { userId: self.id, confirmed: 1, confirmResourceId: null },
    }).then((vote) => {
      return vote ? true : false;
    });
  };
      ...
      if (instance.projectId && !instance.config) {
        db.Project.findByPk(instance.projectId)
          .then((project) => {
            if (!project) {
              return reject(
                new Error(`Project with ID ${instance.projectId} not found`)
              );
            }
            if (!project.config) {
              return reject(
                new Error(`Project with ID ${instance.projectId} has no config`)
              );
            }
