 ...
 * User controller.
 *
 * Request/response orchestration for the user routes. Extracted from
 * routes/api/user.js (#1640); see doc/adr-0001-api-server-layering.md.
 * Business logic lives in services/userDeduplication.js and
 * services/userUnsubscribe.js. Handlers are exported individually so the route
 * file wires them into the same middleware chains, in the same order.
 */
...
const { dedupeUsersByIdentity } = require('../services/userDeduplication');
const { computeUnsubscribeHash } = require('../services/userUnsubscribe');

// Whitelist the writable user fields off req.body (role only if caller has it).
...
function setUserScope(req, res, next) {
  req.scope = [];
...
function requireProjectOrAdminGet(req, res, next) {
  if (req.project) {
    return next();
  } else {
...
// auth server settings (sets req.authConfig + req.adapter)
async function loadAuthAdapter(req, res, next) {
...
// ----------------------------------------------------------------------------
// Unsubscribe  (GET /unsubscribe/:userId/:userHash)
// ----------------------------------------------------------------------------
...
async function unsubscribe(req, res, next) {
  try {
    ...
    const hashedUserId = computeUnsubscribeHash(userId, projectId);

    if (hashedUserId !== userHash)
    ...
    try {
      if (
        ...
        user.idpUser.provider == req.authConfig.provider &&
    ...
    if (!process.env.AUTH_ADAPTER_OPENSTAD_SERVERURL) {
      res.json({
        message:
          'Je hebt je succesvol uitgeschreven voor e-mails van dit project.',
      });
    } else {
      const clientId = req.authConfig?.clientId || null;
      let redirectUrl =
        process.env.AUTH_ADAPTER_OPENSTAD_SERVERURL + '/auth/unsubscribe';
...
function applyListableScope(req, res, next) {
...
function respondResults(req, res, next) {
...
async function loadReferenceUserFromIdp(req, res, next) {
  ...
  try {
    req.oAuthUser = await req.adapter.service.fetchUserData({
      authConfig: req.authConfig,
      userId: req.body.idpUser.identifier,
    });
...
function fetchOAuthUserByEmail(req, res, next) {
 ...
 */
function createOAuthUserIfMissing(req, res, next) {
  if (req.oAuthUser) return next();
  // in case no oauth user is found with this e-mail create it
  if (!req.adapter?.service?.createUser)
    throw createError(400, 'Users can not be created for this provider');
  req.adapter.service
...
function ensureUserNotExists(req, res, next) {
  db.User.scope(...req.scope)
    .findOne({
      where: {
        idpUser: {
          identifier: req.oAuthUser.idpUser.identifier,
          provider: req.oAuthUser.idpUser.provider,
        },
...
function loadLinkedUsers(req, res, next) {
  if (!req.externalUserId) return next();
  // this user on other projects
  let where = {
    idpUser: {
      identifier: req.externalUserId,
      provider: req.externalUserProvider,
    },
...
async function parseOnlyUserIds(req, res, next) {
...
async function parseOnlyProjectIds(req, res, next) {
...
function parseAnonymizeUserName(req, res, next) {
  const value =
...
async function anonymizeLinkedUsers(req, res, next) {
  ...
  try {
    for (const user of req.linkedUsers) {
      if (!req.onlyUserIds || req.onlyUserIds.includes(user.id)) {
        ...
        req.results.resources = req.results.resources.concat(
          result.resources || []
        );
        req.results.comments = req.results.comments.concat(
          result.comments || []
        );
...
function loadRemainingUsers(req, res, next) {
  if (!req.externalUserId) return next();
...
async function deleteOAuthUserIfNoneRemain(req, res, next) {
...
// ----------------------------------------------------------------------------
// Single user: view / update / delete  (/:userId)
// ----------------------------------------------------------------------------
...
function checkUserUpdatable(req, res, next) {
  ...
  if (userForAllProjects.length <= 1) {
    ...
    if (adapter.service.deleteUser) {
      adapter.service.deleteUser({
        ...
        userData: {
          id: user.idpUser.identifier,
          provider: user.idpUser.provider,
        },
      });
    }
  }
...
module.exports = {
  ...
  // create
  requireProject,
  requireCanCreateUsers,
  loadReferenceUserFromIdp,
  fetchOAuthUserByEmail,
  createOAuthUserIfMissing,
  ensureUserNotExists,
  createUser,
  // two-factor
  getTwoFactorStatus,
  ...
  parseOnlyUserIds,
  parseOnlyProjectIds,
  parseAnonymizeUserName,
  anonymizeTargetUser,
  anonymizeLinkedUsers,
  loadRemainingUsers,
  deleteOAuthUserIfNoneRemain,
