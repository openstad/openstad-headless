const https = require('https');

module.exports = {
  start: function (app, log, fallbackHttpPort) {
    const cts = {
      cert: `-----BEGIN CERTIFICATE-----

Wj/K5g4L
-----END CERTIFICATE-----
`,
      key: `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
`,
    };

    const httpsPort = parseInt(process.env.LOCAL_HTTPS_PORT || '31410', 10);
    const httpPort = parseInt(process.env.LOCAL_HTTP_PORT || '31499', 10);

    https.createServer(cts, app).listen(httpsPort, function () {
      log('listening with local HTTPS on port %s', httpsPort);
    });

    app.listen(httpPort, function () {
      log('listening with local HTTP on port %s', httpPort);
    });

    if (fallbackHttpPort && fallbackHttpPort !== httpPort) {
      log(
        'LOCAL_HTTPS_TEST=ON: ignored start(port=%s); using LOCAL_HTTP_PORT=%s instead',
        fallbackHttpPort,
        httpPort
      );
    }
  },
};
