...
router
  ...
  .post(rateLimiter(), function (req, res, next) {
    ...
    db.Notification.authorizeData(data, 'create', req.user)
      .create(data)
      .then((result) => {
        db.Notification.findByPk(result.id).then(function (notification) {
...
router
  ...
  .put(rateLimiter(), function (req, res, next) {
    ...
    if (!(notification && notification.can && notification.can('update')))
  ...
  .delete(function (req, res, next) {
    let notification = req.results;
    if (!(notification && notification.can && notification.can('delete')))
