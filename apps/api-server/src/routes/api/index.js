...
// polls
router.use(
  '/project/:projectId(\\d+)(/resource/:resourceId(\\d+))?/poll',
  require('./poll')
);
...
router.use(
  '/project/:projectId(\\d+)/update-server-login-path/:authProviderId(\\d+)',
  require('./project')
);
router.use(
  '/project/update-server-login-paths-for-auth-provider/:authProviderId(\\d+)',
  require('./project')
);
