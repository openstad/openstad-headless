...
router
    ...
    return db.User.findOne({
      ...
      return db.User.scope(['includeProject'])
        .findAll({
          where: {
            idpUser: {
              identifier: user.idpUser.identifier,
  ...
  .get(function (req, res, next) {
    if (req.activities !== undefined && !req.activities.includes('projects'))
      return next();
    const projectIds = req.users.map((user) => user.projectId);
    let where = { id: projectIds };

    return db.Project.findAll({ where }).then(function (rows) {
  ...
  .get(function (req, res, next) {
    ...
    return db.Resource.scope(['includeProject'])
      .findAll({ where })
      .then(function (rows) {
  ...
  .get(function (req, res, next) {
    if (!req.activities.includes('comments')) return next();
    let where = { userId: req.userIds };
    return db.Comment.scope(['includeResource'])
      .findAll({ where })
      .then(function (rows) {
  ...
  .get(function (req, res, next) {
    if (!req.activities.includes('votes')) return next();
    let where = { userId: req.userIds };
    return db.Vote.scope(['includeResource'])
      .findAll({ where })
      .then(function (rows) {
    ...
    req.activities.forEach((which) => {
      ...
      if (activityConfig[which]) {
        const formattedAsActivities =
          req.results[which] && Array.isArray(req.results[which])
            ? req.results[which].map((instance) => {
                const config = activityConfig[which];
                const resource =
                  which === 'resources' ? instance : instance.resource;
                ...
                return {
                  //strip html tags
                  description: instance[config.descriptionKey]
                    ? instance[config.descriptionKey].replace(
                        /<\/?[^>]+(>|$)/g,
                        ''
                      )
                    : '',
