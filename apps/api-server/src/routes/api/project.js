...
async function updateAllServerLoginPaths(project) {
  ...
  for (const providerId of authProviders) {
    ...
    authProvidersServerLoginPath[providerId] = createServerLoginPath(
      requiredFields,
      provider
    );
  }
...
async function setServerLoginPathForProvider(
  ...
  const requiredFields =
    (project.config.authProvidersRequiredUserFields || {})[authProviderId] ||
    [];
  ...
  config.authProvidersServerLoginPath =
    config.authProvidersServerLoginPath || {};
...
router
  .route('/:projectId/update-server-login-path/:authProviderId')
  .put(async function (req, res, next) {
    const project = await db.Project.scope('includeConfig').findOne({
    ...
    res.json({
      success: true,
      authProvidersServerLoginPath: project.config.authProvidersServerLoginPath,
    });
  });
