  ...
  .post(rateLimiter(), function (req, res, next) {
    db.sequelize
      ...
      .then((transaction) => {
        ...
        return db.Vote.scope(req.scope) // get existing votes for this user
    ...
    if (req.project.config.votes.withExisting == 'merge') {
      ...
      try {
        if (
          ...
          if (transaction) {
            return transaction.commit().finally(() => next(err));
          }
          throw err;
        }
      } catch (err) {
  ...
  .post(function (req, res, next) {
    ...
    if (
      ...
      req.votes.forEach((vote) => {
        ...
        db.Vote.findAll({ where: whereClause, transaction, lock: true })
          .then((found) => {
  ...
  .post(function (req, res, next) {
    ...
    actions.map((action) => {
      switch (action.action) {
        case 'create':
          promises.push(
            db.Vote.create(action.vote, { transaction, lock: true })
          ); // HACK: `upsert` on paranoid deleted row doesn't unset `deletedAt`.
          break;
        case 'update':
          promises.push(
            db.Vote.update(action.vote, {
    ...
    Promise.all(promises)
      .then((result) => {
        req.result = result;
        if (transaction) {
          return transaction.commit().finally(() => result);
        }
        return result;
      })
      .then(() => {
      ...
      .catch((err) => {
        if (transaction) {
          return transaction.rollback().finally(() => next(err));
        }
        next(err);
      });
  })
