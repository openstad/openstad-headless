...
router
  ...
  .get(async function (req, res, next) {
    try {
      ...
      let result = await db.AuthProvider.findAndCountAll({
...
router
  .route('/:id', auth.can('AuthProvider', 'get'))
      ...
      .then((project) => {
        if (project && project.length) {
          ...
          return next(
            new createError(
              400,
              `Deze authenticatie provider wordt nog gebruikt in project: ${ids.join(', ')}.`
            )
          );
        } else {
          provider.destroy().then(() => {
